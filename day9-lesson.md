...
|---|---|
| `<BrowserRouter>` | `main.jsx` wrapping the app |
| `<Routes>` + `<Route>` | `App.jsx` — all 4 routes |
| `<NavLink>` with `isActive` | `Navbar.jsx` — highlights current page |
| `<Link>` | `Home.jsx` — buttons that navigate |
| `useNavigate` | `About.jsx` — button navigates to contact |
| `useNavigate` after submit | `Contact.jsx` — redirect after form submit |
| `path="*"` catch-all | `NotFound.jsx` |

---

## ✅ Day 9 Checklist
- [ ] Know why React apps need a router even though it's one HTML file
- [ ] Can set up `<BrowserRouter>` in `main.jsx`
- [ ] Can define routes with `<Routes>` and `<Route>`
- [ ] Know the difference between `<Link>` and `<a>` tag
- [ ] Can use `<NavLink>` to highlight the active page
- [ ] Can read URL params with `useParams`
- [ ] Can navigate programmatically with `useNavigate`
- [ ] Built the 4-page portfolio app

## 💡 Homework
1. Add a **Projects page** at `/projects`. Display a list of 3–4 fake projects (title, description, tech stack). Each project should be a `<Link>` that goes to `/projects/:id` and shows that project's full details using `useParams`.
2. Add a **footer** component below the routes with your name and the current year. Like the Navbar, it should appear on every page.
3. **Bonus:** Add a **"scroll to top"** effect using `useEffect` — every time the page changes, scroll the window back to the top. Hint: listen for the current location using the `useLocation` hook from React Router.
